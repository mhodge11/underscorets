...
test("creates a new xyz", () => {
	const xyz = new util.XYZ();
	expect(xyz).toBeInstanceOf(util.XYZ);
});
...
test("xyz can be created using the static fromObject method with an object", () => {
	const xyz = util.XYZ.fromObject({ x: 1, y: 2, z: 3 });
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can be created using the static fromObject method with an object with x, y", () => {
	const xyz = util.XYZ.fromObject({ x: 1, y: 2 });
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromObject method with an object with x", () => {
	const xyz = util.XYZ.fromObject({ x: 1 });
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a string", () => {
	const xyz = util.XYZ.fromString("1,2,3");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can be created using the static fromString method with a string with x, y", () => {
	const xyz = util.XYZ.fromString("1,2");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a string with x", () => {
	const xyz = util.XYZ.fromString("1");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString with _ as a separator", () => {
	const xyz = util.XYZ.fromString("1_2_3");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can be created using the static fromString with _ as a separator with x, y", () => {
	const xyz = util.XYZ.fromString("1_2");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a JSON string", () => {
	const xyz = util.XYZ.fromString(JSON.stringify([1, 2, 3]));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can be created using the static fromString method with a JSON string with x, y", () => {
	const xyz = util.XYZ.fromString(JSON.stringify([1, 2]));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a JSON string with x", () => {
	const xyz = util.XYZ.fromString(JSON.stringify([1]));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a JSON string with an empty array", () => {
	const xyz = util.XYZ.fromString(JSON.stringify([]));
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a JSON string with an object", () => {
	const xyz = util.XYZ.fromString(JSON.stringify({ x: 1, y: 2, z: 3 }));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can be created using the static fromString method with a JSON string with an object with x, y", () => {
	const xyz = util.XYZ.fromString(JSON.stringify({ x: 1, y: 2 }));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method with a JSON string with an object with x", () => {
	const xyz = util.XYZ.fromString(JSON.stringify({ x: 1 }));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can be created using the static fromString method prepended with util.XYZ", () => {
	const xyz = util.XYZ.fromString("XYZ [1,2,3]");
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can normalize an xyz array with x, y", () => {
	const xyz = util.XYZ.normalize([1, 2]);
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can normalize an xyz array with x", () => {
	const xyz = util.XYZ.normalize([1]);
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can normalize an xyz instance", () => {
	const xyz = util.XYZ.normalize(new util.XYZ(1, 2, 3));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(3);
});
...
test("xyz can normalize an xyz instance with x, y", () => {
	const xyz = util.XYZ.normalize(new util.XYZ(1, 2));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(2);
	expect(xyz.z).toBe(0);
});
...
test("xyz can normalize an xyz instance with x", () => {
	const xyz = util.XYZ.normalize(new util.XYZ(1));
	expect(xyz.x).toBe(1);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can normalize an xyz instance with no arguments", () => {
	const xyz = util.XYZ.normalize(new util.XYZ());
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with other xyzs", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1, 2, 3), new util.XYZ(4, 5, 6));
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(7);
	expect(xyz.z).toBe(9);
});
...
test("xyz can sum with other xyzs with x, y", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1, 2), new util.XYZ(4, 5));
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(7);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with other xyzs with x", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1), new util.XYZ(4));
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with other xyzs with no arguments", () => {
	const xyz = util.XYZ.sum(new util.XYZ(), new util.XYZ());
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with an xyz array", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1, 2, 3), [4, 5, 6]);
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(7);
	expect(xyz.z).toBe(9);
});
...
test("xyz can sum with an xyz array with x, y", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1, 2), [4, 5]);
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(7);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with an xyz array with x", () => {
	const xyz = util.XYZ.sum(new util.XYZ(1), [4]);
	expect(xyz.x).toBe(5);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can sum with an xyz array with no arguments", () => {
	const xyz = util.XYZ.sum(new util.XYZ(), []);
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can add an xyz to the existing xyz", () => {
	const xyz = new util.XYZ(1, 2, 3);
	xyz.add(new util.XYZ(4, 5, 6), new util.XYZ(1, 1, 0));
	expect(xyz.x).toBe(6);
	expect(xyz.y).toBe(8);
	expect(xyz.z).toBe(9);
});
...
test("xyz can add an xyz to the existing xyz with x, y", () => {
	const xyz = new util.XYZ(1, 2);
	xyz.add(new util.XYZ(4, 5), new util.XYZ(1, 1));
	expect(xyz.x).toBe(6);
	expect(xyz.y).toBe(8);
	expect(xyz.z).toBe(0);
});
...
test("xyz can add an xyz to the existing xyz with x", () => {
	const xyz = new util.XYZ(1);
	xyz.add(new util.XYZ(4), new util.XYZ(1));
	expect(xyz.x).toBe(6);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can add an xyz to the existing xyz with no arguments", () => {
	const xyz = new util.XYZ();
	xyz.add(new util.XYZ(), new util.XYZ());
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can subtract an xyz from the existing xyz", () => {
	const xyz = new util.XYZ(1, 2, 3);
	xyz.subtract(new util.XYZ(4, 5, 6), new util.XYZ(1, 1, 0));
	expect(xyz.x).toBe(-4);
	expect(xyz.y).toBe(-4);
	expect(xyz.z).toBe(-3);
});
...
test("xyz can subtract an xyz from the existing xyz with x, y", () => {
	const xyz = new util.XYZ(1, 2);
	xyz.subtract(new util.XYZ(4, 5), new util.XYZ(1, 1));
	expect(xyz.x).toBe(-4);
	expect(xyz.y).toBe(-4);
	expect(xyz.z).toBe(0);
});
...
test("xyz can subtract an xyz from the existing xyz with x", () => {
	const xyz = new util.XYZ(1);
	xyz.subtract(new util.XYZ(4), new util.XYZ(1));
	expect(xyz.x).toBe(-4);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can subtract an xyz from the existing xyz with no arguments", () => {
	const xyz = new util.XYZ();
	xyz.subtract(new util.XYZ(), new util.XYZ());
	expect(xyz.x).toBe(0);
	expect(xyz.y).toBe(0);
	expect(xyz.z).toBe(0);
});
...
test("xyz can convert an to an util.XYZ representing the sign of each coordinate", () => {
	const xyz = new util.XYZ(-8, 2, 0);
...
test("xyz can convert an to an util.XYZ representing the sign of each coordinate with x, y", () => {
	const xyz = new util.XYZ(-8, 2);
	const xyz2 = xyz.toSign();
	expect(xyz2.x).toBe(-1);
	expect(xyz2.y).toBe(1);
	expect(xyz2.z).toBe(0);
...
test("xyz can convert an to an util.XYZ representing the sign of each coordinate with x", () => {
	const xyz = new util.XYZ(-8);
	const xyz2 = xyz.toSign();
	expect(xyz2.x).toBe(-1);
...
test("xyz can convert an to an util.XYZ representing the sign of each coordinate with no arguments", () => {
	const xyz = new util.XYZ();
	const xyz2 = xyz.toSign();
...
test("xyz can compare an xyz's equality to another xyz", () => {
	const xyz = new util.XYZ(1, 2, 3);
	expect(xyz.eq(new util.XYZ(1, 2, 3))).toBe(true);
	expect(xyz.eq(new util.XYZ(1, 2, 4))).toBe(false);
});
...
test("xyz can get all its neighbors", () => {
	...
	expect(neighbors).toContainEqual(new util.XYZ(1, 1));
	expect(neighbors).toContainEqual(new util.XYZ(0, 2));
	expect(neighbors).toContainEqual(new util.XYZ(2, 2));
	expect(neighbors).toContainEqual(new util.XYZ(1, 3));
});
...
test("xyz can get all its neighbors with diagonals enabled", () => {
	...
	expect(neighbors).toContainEqual(new util.XYZ(2, 2));
	expect(neighbors).toContainEqual(new util.XYZ(1, 3));
	expect(neighbors).toContainEqual(new util.XYZ(0, 1));
	expect(neighbors).toContainEqual(new util.XYZ(2, 1));
	expect(neighbors).toContainEqual(new util.XYZ(0, 3));
	expect(neighbors).toContainEqual(new util.XYZ(2, 3));
});
...
test("xyz can get all its 3d neighbors", () => {
	...
	expect(neighbors).toContainEqual(new util.XYZ(1, 2, 2));
	expect(neighbors).toContainEqual(new util.XYZ(1, 1, 3));
	expect(neighbors).toContainEqual(new util.XYZ(0, 2, 3));
	expect(neighbors).toContainEqual(new util.XYZ(1, 3, 3));
	expect(neighbors).toContainEqual(new util.XYZ(2, 2, 3));
	expect(neighbors).toContainEqual(new util.XYZ(1, 2, 4));
});
...
test("xyz can get all its 3d neighbors with diagonals enabled", () => {
	...
	expect(neighbors).toContainEqual(new util.XYZ(2, 2, 3));
	expect(neighbors).toContainEqual(new util.XYZ(1, 2, 4));
	expect(neighbors).toContainEqual(new util.XYZ(0, 1, 2));
	expect(neighbors).toContainEqual(new util.XYZ(0, 1, 4));
	expect(neighbors).toContainEqual(new util.XYZ(0, 3, 2));
	expect(neighbors).toContainEqual(new util.XYZ(0, 3, 4));
	expect(neighbors).toContainEqual(new util.XYZ(2, 1, 2));
	expect(neighbors).toContainEqual(new util.XYZ(2, 1, 4));
	expect(neighbors).toContainEqual(new util.XYZ(2, 3, 2));
	expect(neighbors).toContainEqual(new util.XYZ(2, 3, 4));
	expect(neighbors).toContainEqual(new util.XYZ(2, 3, 3));
	expect(neighbors).toContainEqual(new util.XYZ(2, 1, 3));
	expect(neighbors).toContainEqual(new util.XYZ(0, 1, 3));
	expect(neighbors).toContainEqual(new util.XYZ(0, 3, 3));
});
...
test("xyz can get distance to another xyz", () => {
	const xyz = new util.XYZ(1, 2, 3);
	expect(xyz.distanceTo(new util.XYZ(1, 2, 3))).toBe(0);
	expect(xyz.distanceTo(new util.XYZ(1, 2, 4))).toBe(1);
	expect(xyz.distanceTo(new util.XYZ(1, 3, 3))).toBe(1);
	expect(xyz.distanceTo(new util.XYZ(2, 2, 3))).toBe(1);
	expect(xyz.distanceTo(new util.XYZ(1, 1, 3))).toBe(1);
	expect(xyz.distanceTo(new util.XYZ(0, 2, 3))).toBe(1);
	expect(xyz.distanceTo(new util.XYZ(1, 2, 2))).toBe(1);
});

test("xyz can get the taxicab distance to another xyz", () => {
	const xyz = new util.XYZ(1, 2, 3);
	expect(xyz.taxicabDistanceTo(new util.XYZ(1, 2, 3))).toBe(0);
	expect(xyz.taxicabDistanceTo(new util.XYZ(1, 2, 4))).toBe(1);
	expect(xyz.taxicabDistanceTo(new util.XYZ(1, 3, 3))).toBe(1);
	expect(xyz.taxicabDistanceTo(new util.XYZ(2, 2, 3))).toBe(1);
	expect(xyz.taxicabDistanceTo(new util.XYZ(1, 1, 3))).toBe(1);
	expect(xyz.taxicabDistanceTo(new util.XYZ(0, 2, 3))).toBe(1);
	expect(xyz.taxicabDistanceTo(new util.XYZ(1, 2, 2))).toBe(1);
});
...
test("xyz can perform a breadth-first search on a 2d coordinate", () => {
	...
	expect(result.visitedPoints).toContainEqual(new util.XYZ(1, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(2, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(1, 3));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(0, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(1, 1));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(3, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(2, 3));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(2, 1));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(1, 4));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(0, 3));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(-1, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(0, 1));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(1, 0));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(4, 2));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(3, 3));
	expect(result.visitedPoints).toContainEqual(new util.XYZ(3, 1));
	expect(result.endPoint).toEqual(new util.XYZ(2, 3));
