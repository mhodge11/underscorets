import { generateUuidAsync, uuidUrlAlphabet } from "@uuid/index.ts";

test("generates URL-friendly IDs", async () => {
	for (let i = 0; i < 100; i++) {
		const id = await generateUuidAsync();
		expect(id.length).toBe(21);
		expectTypeOf(id).toBeString();
	}
});

test("changes ID length", async () => {
	expect(await generateUuidAsync(10)).toHaveLength(10);
});

test("has no collisions", async () => {
...
test("has flat distribution", async () => {
	...
	const LENGTH = (await generateUuidAsync()).length;
