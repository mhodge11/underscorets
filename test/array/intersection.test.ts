...
test("returns the correct intersections", () => {
	...
	const result = array.intersection(arr1, arr2);
...
test("returns the correct intersections select special elements", () => {
	...
	const result = array.intersection(arr1, arr2, (a, b) => a.id === b.id);
...
test("returns unique values", () => {
	...
	const result = array.intersection(arr1, arr2, arr3);
	expect(result).toEqual([3, 4, 5]);
});
...
test("handles 1 missing array", () => {
	...
	const result = array.intersection(arr1, (a, b) => a % 2 === 0 && a === b);
	expect(result).toEqual([]);
});

test("can deal with different types", () => {
	const result = array.intersection([{ id: 1 }, { id: 2 }], [1], (a, b) => {
