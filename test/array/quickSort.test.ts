...
test("quick sort the array in ascending order based on the iteratee function", () => {
	const numbers = [3, 1, 2];
	const sortedNumbers = array.quickSort(numbers, { order: "asc" });
...
test("work with arrays of objects", () => {
	...
	const sortedUsers = array.quickSort(users, {
		order: "desc",
		by: (user) => user.age,
	});
